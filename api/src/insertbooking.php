 ...
 * Update Award
 * 
 * Endpoint for adding a new booking based on the parameters received via POST
 * 
 * @author Razvan Cristian Pintea w20018875
 */
class InsertBooking extends Endpoint {
   
    public function __construct() {
        $data=[];
        
            $data['last_name']=$_POST['renter'];
            $data['pick_up_date']=$_POST['pick_up_date'];
            $data['drop_off_date']=$_POST['drop_off_date'];
            $data['car']=$_POST['car'];
    ...
    protected function initialiseSQL() {
        ...
        $sql="INSERT INTO booking (renter, pick_up_date, drop_off_date, car) 
                      VALUES (:renter, :pick_up_date, :drop_off_date, :car)";
        $this -> setSQL($sql);
        $this -> setSQLParams(['renter'=> $_POST['renter'],
